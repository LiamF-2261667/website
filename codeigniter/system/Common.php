...
if (! function_exists('viewPage')) {
    /**
     * View a page with the default header and footer
     * @param string $name The name of the page to view
     * @param array $data The data to pass to the view
     * @param array $options The options to pass to the view
     * @return string The page
     */
    function viewPage(string $name, array $data = [], array $options = []) : string
    {
        return view('header', $data, $options) . view($name, $data, $options) . view('footer', $data, $options);
    }
}
...
if (! function_exists('currentPage')) {
     ...
     * @return string The current page url (via the default controller routing)
     */
    function currentPage() : string
    {
        $currUrl = "";
        foreach (preg_split("/\//", current_url()) as $key => $value) {
            if ($key > 3) {
                $currUrl .= "/" . $value;
            }
        }
